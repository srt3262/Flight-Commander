...
Flight Commander Firmware 4.1.8 adds the official `CUBEORANGEPLUS` target for
...
The onboard AK09916 is not enabled in 4.1.8. Flight Commander does not yet
...
Commander 4.1.8 does not implement the PX4IO protocol and deliberately keeps
...
4.1.8 image has its vector table at `0x08020000`, code beginning at
...
2. Save a Configurator backup and CLI `diff all` if another firmware is still
   reachable.
3. Select **CubePilot Cube Orange+**, load the official 4.1.8 online image, and
...
- target reports `CUBEORANGEPLUS` and Firmware 4.1.8;
