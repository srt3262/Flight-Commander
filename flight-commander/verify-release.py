#!/usr/bin/env python3
"""Verify the Flight Commander 4.1.5 MICOAIR743 source and HEX contract."""
...
def require_text(path: Path, patterns: list[str]) -> None:
    ...
    for pattern in patterns:
        if not re.search(pattern, text, re.MULTILINE | re.DOTALL):
            fail(f"{path}: required 4.1.5 source contract is missing: {pattern}")
...
def verify_source(root: Path) -> None:
    ...
    require_text(root / "CMakeLists.txt", [
        r"set\(FLIGHT_COMMANDER_FIRMWARE_VERSION 4\.1\.5\)",
    ...
    require_text(root / "src/main/build/flight_commander.h", [
        ...
        r"FLIGHT_COMMANDER_VERSION_PATCH 5",
