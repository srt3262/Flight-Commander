...
def validate_manifest(manifest: dict[str, object], revision: str, tree: str) -> tuple[str, int]:
    ...
    if not version or version != "4.1.5":
        raise RuntimeError(f"Expected Firmware 4.1.5, received {version or 'missing'}")
...
def build(output: Path, build_dir: Path, refresh_manifest: bool) -> dict[str, object]:
    ...
    if refresh_manifest:
        ...
        manifest["artifact"] = {
            "filename": f"Flight-Commander-Firmware-4.1.5-{TARGET}.hex",
            "sha256": "0" * 64,
            "bytes": 0,
        }
