...
$(function() {
    i18n.init( () => {
        ...
        if (store.get('update_notify', false)) {
        ...
        $('a', ui_tabs).on('click', function() {
            ...
            if ($(this).parent().hasClass('active') == false && !GUI.tab_switch_in_progress) { // only initialize when the tab isn't already active
                ...
                var self = this,
                    tabClass = [...$(self).parent()[0].classList]
                        .find((className) => className.startsWith('tab_'));

                var tabRequiresConnection = $(self).closest('ul')
                    .is('.mode-connected, .mode-mavlink, .mode-telemetry');
                ...
                GUI.tab_switch_cleanup(function () {
                    ...
                    switch (tab) {
                       ...
                       case 'javascript_programming':
                           ...
                           break;
                        case 'flight_data':
                            flightDataTab.initialize(content_ready);
                            break;
                        case 'flight_planner':
