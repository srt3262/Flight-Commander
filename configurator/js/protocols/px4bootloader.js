...
export class Px4BootloaderError extends Error {
    constructor(message, options = {}) {
        super(message, options);
        this.name = new.target.name;
        this.code = options.code ?? 'PX4_BOOTLOADER_ERROR';
        this.details = options.details ?? null;
    }
}

export class BootloaderProtocolError extends Px4BootloaderError {
    constructor(message, options = {}) {
...
export class FirmwareCompatibilityError extends Px4BootloaderError {
    constructor(message, options = {}) {
        super(message, { code: 'FIRMWARE_INCOMPATIBLE', ...options });
    }
}

export class FirmwareUploadCancelledError extends Px4BootloaderError {
...
export function padFirmwareImage(input) {
    ...
    if (paddedLength === image.length) return image.slice();
...
export function calculateFirmwareCrc(input, flashSize) {
    ...
    if (!Number.isInteger(flashSize) || flashSize <= 0 || flashSize > 0xffffffff) {
        throw new FirmwareCompatibilityError('Bootloader flash size is invalid.');
    }
    if (flashSize % 4 !== 0) {
        throw new FirmwareCompatibilityError('Bootloader flash size is not four-byte aligned.');
    }
    if (image.length > flashSize) {
        throw new FirmwareCompatibilityError('Firmware image is larger than the writable application flash.');
    }
...
export function checkFirmwareCompatibility(boardInfo, firmware, options = {}) {
    const reasons = [];
    if (!Number.isInteger(boardInfo?.boardId) || boardInfo.boardId <= 0) {
        reasons.push('The bootloader did not provide a valid board ID');
    }
    if (!Number.isInteger(boardInfo?.boardRevision) || boardInfo.boardRevision < 0) {
        reasons.push('The bootloader did not provide a valid board revision');
    }
    if (!Number.isInteger(boardInfo?.flashSize) || boardInfo.flashSize <= 0) {
        reasons.push('The bootloader did not provide a valid flash size');
    }
    if (!Number.isInteger(firmware?.boardId) || firmware.boardId <= 0) {
        reasons.push('The firmware does not have a valid board ID');
    }
    if (!(firmware?.image instanceof Uint8Array) || firmware.image.length === 0) {
        reasons.push('The firmware image is empty or invalid');
    }

    if (reasons.length === 0) {
        const aliases = new Set(options.compatibleBoardIds?.[boardInfo.boardId] ?? []);
        if (boardInfo.boardId !== firmware.boardId && !aliases.has(firmware.boardId)) {
            reasons.push(
                `Firmware board ID ${firmware.boardId} does not match controller board ID ${boardInfo.boardId}`,
            );
        }
        if (firmware.image.length > boardInfo.flashSize) {
            reasons.push(
                `Firmware image (${firmware.image.length} bytes) exceeds controller application flash (${boardInfo.flashSize} bytes)`,
            );
        }
    }
