...
The complete Configurator source test suite passed after the Flight Commander-only product-surface cleanup. Publication also rebuilds the retained firmware source archive and packages and verifies the Windows x64 application before creating the release. The packaged-renderer verifier now requires the Flight Commander-only flashing warning and rejects the retired compatibility wording.
