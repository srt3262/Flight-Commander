...
#if defined(USE_GPS_PROTO_DRONECAN)
gpsSolutionData_t gpsUartSol;
static gpsPrimarySource_e pendingGpsSource = GPS_PRIMARY_SOURCE_UART;
#endif
...
{
#if defined(USE_GPS_PROTO_DRONECAN)
    gpsUartSol = gpsSolDRV;
    pendingGpsSource = GPS_PRIMARY_SOURCE_UART;
    const bool uartIsActive = gpsGetPrimarySource() == GPS_PRIMARY_SOURCE_UART || !gpsDronecanIsHealthy();
    if (uartIsActive) {
        gpsSol = gpsUartSol;
    }
#else
...
{
#if defined(USE_GPS_PROTO_DRONECAN)
    const bool sourceIsPrimary =
        (pendingGpsSource == GPS_PRIMARY_SOURCE_UART &&
            (gpsGetPrimarySource() == GPS_PRIMARY_SOURCE_UART || !gpsDronecanIsHealthy())) ||
        (pendingGpsSource == GPS_PRIMARY_SOURCE_DRONECAN &&
            (gpsGetPrimarySource() == GPS_PRIMARY_SOURCE_DRONECAN || !gpsUartIsHealthy()));

    if (!sourceIsPrimary) {
        if (!timeout && pendingGpsSource == GPS_PRIMARY_SOURCE_UART) {
    ...
    if (!timeout) {
#if defined(USE_GPS_PROTO_DRONECAN)
        if (pendingGpsSource == GPS_PRIMARY_SOURCE_UART) {
#endif
            gpsSetProtocolTimeout(gpsState.baseTimeoutMs);
            gpsStats.lastMessageDt = gpsState.lastMessageMs - gpsState.lastLastMessageMs;
#if defined(USE_GPS_PROTO_DRONECAN)
...
gpsPrimarySource_e gpsGetPrimarySource(void)
{
#if defined(USE_GPS_PROTO_DRONECAN)
    if (dronecanConfig()->primaryGpsSource < GPS_PRIMARY_SOURCE_COUNT) {
        return (gpsPrimarySource_e)dronecanConfig()->primaryGpsSource;
    }
#endif
    return GPS_PRIMARY_SOURCE_UART;
}

#if defined(USE_GPS_PROTO_DRONECAN)
bool gpsUartIsEnabled(void)
{
    return feature(FEATURE_GPS) && gpsConfig()->provider == GPS_UBLOX && gpsState.gpsPort;
}

uint32_t gpsUartAgeMs(void)
{
    return gpsState.lastMessageMs ? millis() - gpsState.lastMessageMs : UINT32_MAX;
}

bool gpsUartIsHealthy(void)
{
    return gpsUartIsEnabled() && gpsState.state == GPS_RUNNING && gpsUartAgeMs() <= gpsState.baseTimeoutMs;
}

void gpsProcessNewDronecanData(void)
{
    pendingGpsSource = GPS_PRIMARY_SOURCE_DRONECAN;
    const bool dronecanIsActive = gpsGetPrimarySource() == GPS_PRIMARY_SOURCE_DRONECAN || !gpsUartIsHealthy();
    if (dronecanIsActive) {
        gpsSol = gpsDronecanSol;
#ifdef USE_GPS_FIX_ESTIMATION
        processDisableGPSFix();
        updateEstimatedGPSFix();
#endif
...
#if defined(USE_GPS_PROTO_DRONECAN)
    if (!gpsState.gpsPort && gpsDronecanIsEnabled()) {
        if (!gpsDronecanIsHealthy()) {
            sensorsClear(SENSOR_GPS);
            DISABLE_STATE(GPS_FIX);
            gpsTryEstimateOnTimeout();
        }
        const bool result = gpsSol.flags.hasNewData;
        gpsSol.flags.hasNewData = false;
        return result;
    }
#endif
...
bool gpsQueueRtcmData(const uint8_t *data, uint16_t length)
{
    if (!data || !length || !gpsState.gpsPort || gpsConfig()->provider != GPS_UBLOX) {
        return false;
    }
    if (serialTxBytesFree(gpsState.gpsPort) < length) {
        return false;
    }
    serialWriteBuf(gpsState.gpsPort, data, length);
    return true;
}

uint16_t gpsGetRtcmPendingBytes(void)
{
    if (!gpsState.gpsPort || gpsState.gpsPort->txBufferSize <= 1) {
        return 0;
    }
    const uint32_t capacity = gpsState.gpsPort->txBufferSize - 1U;
    const uint32_t freeBytes = serialTxBytesFree(gpsState.gpsPort);
    return freeBytes < capacity ? capacity - freeBytes : 0;
}
