#!/usr/bin/env python3
"""Structural verification for the Flight Commander 4.0.8 compass release build."""
...
EXPECTED_IDENTITY = b"FCFW" + bytes((1, 4, 0, 8, 9, 1, 0, 0xFF, 0xFF, 0, 0))
...
def main() -> int:
    ...
    if not contains(memory, EXPECTED_IDENTITY):
        fail("HEX does not contain the Flight Commander 4.0.8 identity and 0x0000ffff capability mask")
    if not contains(memory, EXPECTED_TARGET):
        fail("HEX does not contain the MICOAIR743 target identity")

    digest = hashlib.sha256(path.read_bytes()).hexdigest()
    print(f"Verified Flight Commander 4.0.8 MICOAIR743 HEX: {path}")
