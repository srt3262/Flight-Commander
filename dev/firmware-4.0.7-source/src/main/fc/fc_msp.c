...
#endif
#ifdef USE_FLIGHT_COMMANDER_RTK
...
static bool mspFcProcessOutCommand(uint16_t cmdMSP, sbuf_t *dst, mspPostProcessFnPtr *mspPostProcessFn)
{
    ...
    case MSP2_FLIGHT_COMMANDER_INFO:
        mspFlightCommanderWriteInfo(dst);
        break;

#ifdef USE_FLIGHT_COMMANDER_RTK
    case MSP2_FLIGHT_COMMANDER_RTK_STATUS: {
        const flightCommanderRtcmStatus_t *status = flightCommanderRtcmGetStatus();
        sbufWriteU8(dst, status->transport);
        sbufWriteU8(dst, gpsSol.fixType);
        sbufWriteU16(dst, gpsGetRtcmPendingBytes());
        sbufWriteU32(dst, status->receivedPackets);
        sbufWriteU32(dst, status->completedMessages);
        sbufWriteU32(dst, status->injectedBytes);
        sbufWriteU32(dst, status->invalidPackets);
        sbufWriteU32(dst, status->incompleteMessages);
        sbufWriteU32(dst, status->queueDrops);
        break;
    }
#endif

#ifdef USE_DRONECAN
    case MSP2_FLIGHT_COMMANDER_DUAL_GPS_STATUS: {
        uint8_t flags = 0;
        if (gpsUartIsEnabled()) flags |= 1U << 0;
        if (gpsUartIsHealthy()) flags |= 1U << 1;
        if (gpsDronecanIsEnabled()) flags |= 1U << 2;
        if (gpsDronecanIsHealthy()) flags |= 1U << 3;
        if (gpsUartSol.fixType >= GPS_FIX_RTK_FLOAT) flags |= 1U << 4;
        if (gpsDronecanSol.fixType >= GPS_FIX_RTK_FLOAT) flags |= 1U << 5;
        sbufWriteU8(dst, flags);
        sbufWriteU8(dst, gpsGetPrimarySource());
        sbufWriteU8(dst, gpsConfig()->provider);
        sbufWriteU8(dst, gpsUartSol.fixType);
        sbufWriteU8(dst, gpsUartSol.numSat);
        sbufWriteU8(dst, gpsDronecanNodeId());
        sbufWriteU8(dst, gpsDronecanSol.fixType);
        sbufWriteU8(dst, gpsDronecanSol.numSat);
        sbufWriteU32(dst, gpsUartSol.llh.lat);
        sbufWriteU32(dst, gpsUartSol.llh.lon);
        sbufWriteU32(dst, gpsUartSol.llh.alt);
        sbufWriteU32(dst, gpsDronecanSol.llh.lat);
        sbufWriteU32(dst, gpsDronecanSol.llh.lon);
        sbufWriteU32(dst, gpsDronecanSol.llh.alt);
        sbufWriteU32(dst, gpsDronecanAgeMs());
#ifdef USE_FLIGHT_COMMANDER_HEADING_FUSION
        const flightCommanderHeadingStatus_t *heading = flightCommanderHeadingGetStatus();
        sbufWriteU16(dst, heading->baselineHeadingCentidegrees);
        sbufWriteU16(dst, heading->baselineDistanceCm);
#else
        sbufWriteU16(dst, 0);
        sbufWriteU16(dst, 0);
#endif
    ...
    case MSP2_FLIGHT_COMMANDER_DRONECAN_NODES:
        sbufWriteU8(dst, dronecanGetState());
        sbufWriteU16(dst, dronecanGetBitrateKbps());
        sbufWriteU8(dst, dronecanGetNodeCount());
        for (unsigned index = 0; index < dronecanGetNodeCount(); index++) {
            const dronecanNodeInfo_t *node = dronecanGetNode(index);
            sbufWriteU8(dst, node->nodeID);
            sbufWriteU8(dst, node->health);
            sbufWriteU8(dst, node->mode);
            const uint32_t ageSeconds = (millis() - node->last_seen_ms) / 1000U;
            sbufWriteU16(dst, ageSeconds > UINT16_MAX ? UINT16_MAX : (uint16_t)ageSeconds);
            sbufWriteU16(dst, node->capabilities);
        }
        break;
#endif

#ifdef USE_FLIGHT_COMMANDER_HEADING_FUSION
    case MSP2_FLIGHT_COMMANDER_HEADING_CONFIG:
        flightCommanderHeadingWriteConfig(dst);
        break;

    case MSP2_FLIGHT_COMMANDER_HEADING_STATUS:
        flightCommanderHeadingWriteStatus(dst);
        break;
#endif
...
static mspResult_e mspFcProcessInCommand(uint16_t cmdMSP, sbuf_t *src)
{
    ...
    switch (cmdMSP) {
#ifdef USE_FLIGHT_COMMANDER_RTK
    case MSP2_FLIGHT_COMMANDER_RTCM_DATA: {
        if (dataSize < 2) {
            return MSP_RESULT_ERROR;
        }
        const uint8_t flags = sbufReadU8(src);
        const uint8_t length = sbufReadU8(src);
        if (length != sbufBytesRemaining(src) || length > 180) {
            return MSP_RESULT_ERROR;
        }
        flightCommanderRtcmReceiveFragment(FLIGHT_COMMANDER_RTCM_TRANSPORT_MSP,
            flags, length, sbufPtr(src));
...
#ifdef USE_FLIGHT_COMMANDER_HEADING_FUSION
    case MSP2_FLIGHT_COMMANDER_SET_HEADING_CONFIG:
        if (ARMING_FLAG(ARMED) || !flightCommanderHeadingReadConfig(src)) {
            return MSP_RESULT_ERROR;
        }
        break;
#endif
