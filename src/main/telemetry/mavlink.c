...
void mavlinkSendPosition(timeUs_t currentTimeUs)
{
    ...
    if (gpsSol.fixType == GPS_NO_FIX) {
        ...
        gpsFixType = 3;
    } else if (gpsSol.fixType == GPS_FIX_RTK_FLOAT) {
        gpsFixType = 5;
    } else if (gpsSol.fixType == GPS_FIX_RTK_FIXED) {
        gpsFixType = 6;
    }
...
#ifdef USE_FLIGHT_COMMANDER_RTK
static void handleIncoming_GPS_RTCM_DATA(void)
{
    mavlink_gps_rtcm_data_t message;
    mavlink_msg_gps_rtcm_data_decode(&mavRecvMsg, &message);
    if (message.len <= sizeof(message.data)) {
        flightCommanderRtcmReceiveFragment(FLIGHT_COMMANDER_RTCM_TRANSPORT_MAVLINK,
            message.flags, message.len, message.data);
    }
}
#endif
        ...
        if (result == MAVLINK_FRAMING_OK) {
            switch (mavRecvMsg.msgid) {
                ...
                case MAVLINK_MSG_ID_GPS_RTCM_DATA:
                    handleIncoming_GPS_RTCM_DATA();
                    // Correction packets must not starve normal telemetry scheduling.
                    return false;
#endif
                default:
                    return false;
