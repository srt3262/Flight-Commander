...
int16_t canardSTM32Transmit(const CanardCANFrame* const tx_frame) {
    ...
    {
        LOG_ERROR(SYSTEM, "DroneCAN data exceeds the classic CAN frame size");
        return -CANARD_ERROR_INVALID_ARGUMENT;
    }
...
int16_t canardSTM32CAN1_Init(uint32_t bitrate)
{
    ...
    /* Select the FDCAN kernel clock before deriving nominal bit timings. */
    PeriphClkInitStruct.PeriphClockSelection = RCC_PERIPHCLK_FDCAN;
    PeriphClkInitStruct.FdcanClockSelection = RCC_FDCANCLKSOURCE_PLL;
    if (HAL_RCCEx_PeriphCLKConfig(&PeriphClkInitStruct) != HAL_OK)
    {
      LOG_DEBUG(SYSTEM, "Unable to configure DroneCAN peripheral clock");
    ...
    if (HAL_FDCAN_ConfigGlobalFilter(&hfdcan1, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_ACCEPT_IN_RX_FIFO0, FDCAN_FILTER_REMOTE, FDCAN_FILTER_REMOTE) != HAL_OK) {
        LOG_ERROR(SYSTEM, "Failed to configure the DroneCAN filter");
        return -CANARD_ERROR_INTERNAL;
    }

    if (HAL_FDCAN_Start(&hfdcan1) != HAL_OK) {
        LOG_ERROR(SYSTEM, "Failed to start DroneCAN");
        return -CANARD_ERROR_INTERNAL;
    }
    return CANARD_OK;
}
...
static void canardSTM32GPIO_Init(void)
{
   // Set up the Rx and Tx pins for CAN1 and if present, the standby or listen only pin.
    ...
    IOInit(IOGetByTag(IO_TAG(CAN1_TX)), OWNER_SYSTEM, RESOURCE_OUTPUT, 0);
    IOConfigGPIOAF(IOGetByTag(IO_TAG(CAN1_TX)), IOCFG_AF_PP, GPIO_AF9_FDCAN1);  // How do I make the alternate function crossplatform?
    IOInit(IOGetByTag(IO_TAG(CAN1_RX)), OWNER_SYSTEM, RESOURCE_INPUT, 0);
    ...
    IOInit(IOGetByTag(IO_TAG(CAN1_STANDBY)), OWNER_SYSTEM, RESOURCE_OUTPUT, 0);
...
static bool canardSTM32ComputeTimings(const uint32_t target_bitrate, struct Timings *out_timings)
{
    ...
    LOG_DEBUG(SYSTEM, "DroneCAN baudrate: %lu", target_bitrate);
    ...
    /* ISO 11898 requires SJW not to exceed phase segment 2. */
    out_timings->sjw = solution.bs2 < 4U ? solution.bs2 : 4U;
...
void canardSTM32GetProtocolStatus(canardProtocolStatus_t *pProtocolStat){
    ...
    LOG_DEBUG(SYSTEM, "DroneCAN bus-off: %lu", protocolStatus.BusOff);
