...
#define DSHOT_DMA_CACHE_ALIGNMENT 32
#define DSHOT_DMA_BUFFER_ALIGNED_WORDS (((GCR_TELEMETRY_INPUT_LEN * sizeof(timerDMASafeType_t) + DSHOT_DMA_CACHE_ALIGNMENT - 1) / DSHOT_DMA_CACHE_ALIGNMENT) * DSHOT_DMA_CACHE_ALIGNMENT / sizeof(timerDMASafeType_t))
#define DSHOT_DMA_BUFFER_ALIGNED_BYTES (DSHOT_DMA_BUFFER_ALIGNED_WORDS * sizeof(timerDMASafeType_t))
...
typedef enum {
    DSHOT_DMA_PHASE_OUTPUT = 0,
    DSHOT_DMA_PHASE_INPUT,
    DSHOT_DMA_PHASE_CAPTURE_COMPLETE,
} dshotDmaPhase_e;

#if defined(AT32F43x)
#define DSHOT_DMA_PHASE_CLEAR_FLAGS (DMA_IT_TCIF | DMA_IT_HTIF | DMA_IT_DMEIF)
#else
#define DSHOT_DMA_PHASE_CLEAR_FLAGS (DMA_IT_TCIF | DMA_IT_HTIF | DMA_IT_TEIF)
#endif
#endif
...
#ifdef USE_DSHOT
static DMA_RAM timerDMASafeType_t dshotDmaBuffers[MAX_PWM_OUTPUTS][DSHOT_DMA_BUFFER_ALIGNED_WORDS] __attribute__((aligned(DSHOT_DMA_CACHE_ALIGNMENT)));
#endif
...
#endif
    volatile dshotDmaPhase_e dshotDmaPhase;
...
static pwmOutputPort_t pwmOutputPorts[MAX_PWM_OUTPUTS];
...
static volatile bool dshotTelemetryPending = false;
...
static bool pwmDshotSetDirectionOutput(pwmOutputPort_t *port);
static bool pwmDshotSetDirectionInput(pwmOutputPort_t *port);
static void pwmDshotPrepareTimersForOutput(void);
...
#ifdef USE_DSHOT
    p->dmaBuffer = dshotDmaBuffers[allocatedOutputPortCount];
#endif
...
void pwmSetMotorDMACircular(bool circular)
{
    ...
    // Bidirectional DShot reuses each channel DMA stream for input capture.
    // It cannot be made circular without either repeating telemetry requests
    // while still driving the line or corrupting an in-progress capture. A
    // configuration save is disarmed, so leave the normal one-shot state
    // untouched and let the ESC fail closed on the brief signal interruption.
...
static uint16_t dshotDecodeTelemetryPacket(const uint32_t buffer[], uint32_t count)
{
    if (count <= MIN_GCR_EDGES || count > GCR_TELEMETRY_INPUT_LEN) {
...
static bool pwmDshotOutputIdleHigh(const pwmOutputPort_t *port)
{
    // Bidirectional DShot uses the inverse of the target's normal output
    // polarity. With PWM1 and CCR=0 this is also the physical idle level.
    return (port->tch->timHw->output & TIMER_OUTPUT_INVERTED) == 0;
}

static bool pwmDshotSetDirectionOutput(pwmOutputPort_t *port)
{
#if defined(USE_HAL_DRIVER)
    const bool outputIdleHigh = pwmDshotOutputIdleHigh(port);
#if defined(STM32H7)
    ...
    init.OCPolarity = outputIdleHigh ? TIM_OCPOLARITY_LOW : TIM_OCPOLARITY_HIGH;
    init.OCNIdleState = TIM_OCNIDLESTATE_SET;
    init.OCNPolarity = outputIdleHigh ? TIM_OCNPOLARITY_LOW : TIM_OCNPOLARITY_HIGH;
    init.Pulse = 0;
    init.OCFastMode = TIM_OCFAST_DISABLE;
    if (HAL_TIM_PWM_ConfigChannel(port->tch->timCtx->timHandle, &init, dshotTimChannel(port)) != HAL_OK) {
        return false;
    }
    // Do not use HAL_TIM_PWM_Start here. The channel is already BUSY from
    // initial motor setup and HAL would reject the direction change. Enabling
    // CCx directly also avoids stopping a timer shared by other motors.
    timerChCaptureEnable(port->tch);
...
#if defined(STM32H7)
    IOConfigGPIOAF(io, outputIdleHigh ? IOCFG_AF_PP_UP : IOCFG_AF_PP_PD, port->tch->timHw->alternateFunction);
#endif
#elif defined(AT32F43x)
    ...
    port->dshotDmaPhase = DSHOT_DMA_PHASE_OUTPUT;
    return true;
}
...
static bool pwmDshotSetDirectionInput(pwmOutputPort_t *port)
{
    ...
    port->dshotDmaPhase = DSHOT_DMA_PHASE_INPUT;
    ...
    IOConfigGPIO(io, IOCFG_OUT_PP);
#endif
    LL_TIM_EnableARRPreload(port->tch->timHw->tim);
    LL_TIM_SetAutoReload(port->tch->timHw->tim, 0xffffffffU);
    ...
    init.ICFilter = 2;
    if (HAL_TIM_IC_ConfigChannel(port->tch->timCtx->timHandle, &init, dshotTimChannel(port)) != HAL_OK) {
        port->dshotDmaPhase = DSHOT_DMA_PHASE_CAPTURE_COMPLETE;
        return false;
    }
    // See the output-direction path: HAL_TIM_IC_Start requires READY even
    // though the PWM setup intentionally leaves the shared channel BUSY.
    timerChCaptureEnable(port->tch);
...
static void pwmDshotPrepareTimersForOutput(void)
{
    ...
    for (int motorIndex = 0; motorIndex < getMotorCount(); motorIndex++) {
        ...
        bool timerAlreadyPrepared = false;
        for (int previousMotor = 0; previousMotor < motorIndex; previousMotor++) {
            pwmOutputPort_t *previousPort = motors[previousMotor].pwmPort;
            if (previousPort && previousPort->configured && previousPort->tch->timHw->tim == port->tch->timHw->tim) {
                timerAlreadyPrepared = true;
                break;
            }
        }

        if (!timerAlreadyPrepared) {
            TIM_TypeDef *timer = port->tch->timHw->tim;
            LL_TIM_DisableARRPreload(timer);
            LL_TIM_SetAutoReload(timer, DSHOT_MOTOR_BITLENGTH - 1);
...
static void pwmDshotDmaIrqHandler(DMA_t descriptor)
{
    ...
    const dshotDmaPhase_e completedPhase = port->dshotDmaPhase;
    ...
    // Clear the completed phase before a stream is reconfigured and enabled.
    // Otherwise the output TC flag can immediately retrigger as an input TC.
    DMA_CLEAR_FLAG(descriptor, DSHOT_DMA_PHASE_CLEAR_FLAGS);

    if (useDshotTelemetry && completedPhase == DSHOT_DMA_PHASE_OUTPUT) {
        if (!pwmDshotSetDirectionInput(port)) {
            port->dshotDmaPhase = DSHOT_DMA_PHASE_CAPTURE_COMPLETE;
        }
        dshotTelemetryPending = true;
    } else if (completedPhase == DSHOT_DMA_PHASE_INPUT) {
        port->dshotDmaPhase = DSHOT_DMA_PHASE_CAPTURE_COMPLETE;
    }
}
...
static bool NOINLINE pwmDshotDecodeTelemetry(void)
{
    ...
    for (int motorIndex = 0; motorIndex < getMotorCount(); motorIndex++) {
        pwmOutputPort_t *port = motors[motorIndex].pwmPort;
        if (!port || !port->configured || port->dshotDmaPhase == DSHOT_DMA_PHASE_OUTPUT) {
            continue;
        }
        ...
        ATOMIC_BLOCK(NVIC_PRIO_MAX) {
            ...
            LL_DMA_DisableStream(dmaBase, streamLL);
            port->dshotDmaPhase = DSHOT_DMA_PHASE_CAPTURE_COMPLETE;
            DMA_CLEAR_FLAG(port->tch->dma, DSHOT_DMA_PHASE_CLEAR_FLAGS);
        }

        // EN clears asynchronously on STM32 DMA. Do not invalidate or read a
        // cache line while the stream can still write capture data into it.
        ...
        edges = GCR_TELEMETRY_INPUT_LEN - LL_DMA_GetDataLength(dmaBase, streamLL);
#if defined(STM32H7)
        SCB_InvalidateDCache_by_Addr((uint32_t *)port->dmaBuffer, DSHOT_DMA_BUFFER_ALIGNED_BYTES);
#endif
#elif defined(AT32F43x)
        ATOMIC_BLOCK(NVIC_PRIO_MAX) {
            tmr_dma_request_enable(port->tch->timHw->tim, dshotDmaSource(port), FALSE);
        ...
        if (!pwmDshotSetDirectionOutput(port)) {
            return false;
        }
    }
...
{
    if (useDshotTelemetry && (timerHardware->output & TIMER_OUTPUT_N_CHANNEL)) {
        LOG_ERROR(PWM, "Bidirectional DShot requires a non-complementary timer channel");
        return NULL;
    }

#if defined(STM32H7) && defined(USE_DSHOT_DMAR)
    // STM32H7 has no TIM4_CH4 DMAMUX request. Unidirectional DMAR uses the
    // TIM4 update request as a target-level workaround, but bidirectional
    // capture requires the real per-channel request used below.
    if (useDshotTelemetry && timerHardware->tim == TIM4 && timerHardware->channelIndex == 3) {
        LOG_ERROR(PWM, "Motor output has no dedicated DMA channel for DShot");
        return NULL;
    }
#endif
    ...
    if (enableOutput && useDshotTelemetry) {
        IOConfigGPIOAF(IOGetByTag(timerHardware->tag), pwmDshotOutputIdleHigh(port) ? IOCFG_AF_PP_UP : IOCFG_AF_PP_PD, timerHardware->alternateFunction);
    }
...
#endif
    {
        ...
        memset(port->dmaBuffer, 0, DSHOT_DMA_BUFFER_ALIGNED_BYTES);
        port->configured = true;
            port->dshotTelemetryDeadtimeUs = DSHOT_TELEMETRY_DEADTIME_US + 1000000 * (16 * DSHOT_MOTOR_BITLENGTH) / dshotHz;
            if (useDshotTelemetry && !pwmDshotSetDirectionOutput(port)) {
...
void pwmCompleteMotorUpdate(void) {
    ...
    if (isMotorProtocolDshot()) {
        ...
        {
        if (useDshotTelemetry) {
            pwmDshotPrepareTimersForOutput();
        }
...
#if defined(STM32H7)
                SCB_CleanDCache_by_Addr((uint32_t *)motors[index].pwmPort->dmaBuffer, DSHOT_DMA_BUFFER_ALIGNED_BYTES);
#endif
...
void pwmMotorPreconfigure(void)
{
    ...
    useDshotTelemetry = feature(FEATURE_PWM_OUTPUT_ENABLE)
        ...
        && getMotorProtocolProperties(initMotorProtocol)->isDSHOT;
#endif
