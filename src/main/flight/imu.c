...
#if defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
    flightCommanderHeadingSetMagneticNorth(&vCorrectedMagNorth);
#endif
}
...
#if defined(USE_GPS) || defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
...
#if defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
static bool imuCalculateFlightCommanderMagError(
    fpVector3_t *errorBody,
    const fpVector3_t *magneticFieldBody)
{
    if (vectorNormSquared(magneticFieldBody) <= 0.01F) {
        return false;
    }

    fpVector3_t magneticFieldEarth;
    quaternionRotateVectorInv(&magneticFieldEarth, magneticFieldBody, &orientation);
    magneticFieldEarth.z = 0.0F;
    if (vectorNormSquared(&magneticFieldEarth) <= 0.01F) {
        return false;
    }
    vectorNormalize(&magneticFieldEarth, &magneticFieldEarth);

    fpVector3_t errorEarth;
    // This is the same declination-aware yaw error used by the unmodified
    // INAV onboard-compass path below.
    vectorCrossProduct(&errorEarth, &magneticFieldEarth, &vCorrectedMagNorth);
    quaternionRotateVector(errorBody, &errorEarth, &orientation);
    return true;
}

static bool imuCalculateFlightCommanderAbsoluteError(
    fpVector3_t *errorBody,
    const fpVector3_t *referenceHeadingEarth)
{
    // A moving-baseline solution is an absolute mounting reference, not a
    // course-over-ground estimate.  Vehicle heading is always body X/front.
    // INAV's COG path deliberately uses body Z for a multirotor so tilt can
    // indicate acceleration direction; copying that special case here made a
    // level aircraft's absolute yaw reference collapse to a zero XY vector.
    const fpVector3_t forwardBody = { .v = { 1.0F, 0.0F, 0.0F } };

    fpVector3_t estimatedHeadingEarth;
    quaternionRotateVectorInv(&estimatedHeadingEarth, &forwardBody, &orientation);
    estimatedHeadingEarth.z = 0.0F;
    if (vectorNormSquared(&estimatedHeadingEarth) <= 0.01F) {
        return false;
    }
    vectorNormalize(&estimatedHeadingEarth, &estimatedHeadingEarth);

    fpVector3_t errorEarth;
    // Match INAV's COG correction sign and coordinate convention exactly.
    vectorCrossProduct(&errorEarth, referenceHeadingEarth, &estimatedHeadingEarth);
    quaternionRotateVector(errorBody, &errorEarth, &orientation);
    return true;
}
#endif
...
#if defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
    const bool hasFlightCommanderHeading = flightCommanderHeadingHasActiveReference();
#else
    const bool hasFlightCommanderHeading = false;
#endif
    if (magBF || vCOG || vCOGAcc || hasFlightCommanderHeading) {
        float wMag = 0.0f;
        float wCoG = 1.0f;
...
#if defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
            // Source quality already contains INAV's field-magnitude nearness.
            // Applying the bell curve again would square the quality and make
            // a usable compass correct yaw far too weakly near the threshold.
            wMag = flightCommanderHeadingGetOnboardMagWeight();
#else
        ...
        fpVector3_t vFlightCommanderErr = { .v = { 0.0f, 0.0f, 0.0f } };
        float flightCommanderWeight = 0.0F;
#if defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
        for (unsigned sourceIndex = FLIGHT_COMMANDER_HEADING_EXTERNAL_I2C_MAG;
            sourceIndex <= FLIGHT_COMMANDER_HEADING_DRONECAN_MAG; sourceIndex++) {
            const flightCommanderHeadingSource_e source =
                (flightCommanderHeadingSource_e)sourceIndex;
            fpVector3_t sourceFieldBody;
            float sourceWeight;
            if (!flightCommanderHeadingGetMagSource(
                    source, &sourceFieldBody, &sourceWeight)) {
                continue;
            }
            fpVector3_t sourceError;
            if (sourceWeight > 0.0F &&
                imuCalculateFlightCommanderMagError(&sourceError, &sourceFieldBody)) {
                vectorScale(&sourceError, &sourceError, sourceWeight);
                vectorAdd(&vFlightCommanderErr, &vFlightCommanderErr, &sourceError);
                flightCommanderWeight += sourceWeight;
            }
        }

        fpVector3_t absoluteHeadingEarth;
        float absoluteWeight;
        fpVector3_t absoluteError;
        if (flightCommanderHeadingGetAbsoluteReference(
                &absoluteHeadingEarth, &absoluteWeight) &&
            imuCalculateFlightCommanderAbsoluteError(
                &absoluteError, &absoluteHeadingEarth)) {
            vectorScale(&absoluteError, &absoluteError, absoluteWeight);
            vectorAdd(&vFlightCommanderErr, &vFlightCommanderErr, &absoluteError);
            flightCommanderWeight += absoluteWeight;
        }
#endif
        ...
        vectorAdd(&vMagErr, &vMagErr, &vFlightCommanderErr);
        const float absoluteWeightSum = wMag + flightCommanderWeight;
        if (absoluteWeightSum > 1.0F) {
            vectorScale(&vMagErr, &vMagErr, 1.0F / absoluteWeightSum);
        }
...
#if defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
        && flightCommanderHeadingGetOnboardMagWeight() > 0.0F
#endif
        ;
#else
...
#if defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
        || flightCommanderHeadingHasActiveReference()
#endif
        ;
}
