...
#if defined(USE_FLIGHT_COMMANDER_MOVING_BASELINE)
...
static bool gpsParseFrameUBLOX(void)
{
    ...
    case MSG_PVT:
        ...
        if (next_fix_type == GPS_FIX_3D) {
            const uint8_t carrierSolution = (_buffer.pvt.reserved1 >> 6) & 0x03;
            if (carrierSolution == 1) {
                next_fix_type = GPS_FIX_RTK_FLOAT;
            } else if (carrierSolution == 2) {
                next_fix_type = GPS_FIX_RTK_FIXED;
            }
        }
        gpsSolDRV.fixType = next_fix_type;
...
#if defined(USE_FLIGHT_COMMANDER_MOVING_BASELINE)
    case MSG_RELPOSNED:
        if (_class == CLASS_NAV && _payload_length == sizeof(ubx_nav_relposned) &&
            _buffer.relposned.version == 1U) {
            flightCommanderHeadingReceiveUartRelPosHeading(
                _buffer.relposned.relative_heading,
                _buffer.relposned.accuracy_heading,
                _buffer.relposned.relative_length,
                _buffer.relposned.relative_hp_length,
                _buffer.relposned.flags);
        }
        break;
#endif
    case MSG_VER:
...
#if defined(USE_FLIGHT_COMMANDER_MOVING_BASELINE)
    const flightCommanderHeadingConfig_t *headingConfig = flightCommanderHeadingConfig();
    if (headingConfig->movingBaselineEnabled &&
        headingConfig->movingBaselineProvider != FLIGHT_COMMANDER_BASELINE_DRONECAN) {
        gpsSetProtocolTimeout(GPS_SHORT_TIMEOUT);
        if (ubloxVersionGT(23, 1)) {
            ubx_config_data8_payload_t relativeHeadingRate[] = {
                {UBLOX_CFG_MSGOUT_NAV_RELPOSNED_UART1, 1},
            };
            ubloxSendSetCfgBytes(relativeHeadingRate, 1);
            ptWaitTimeout((_ack_state == UBX_ACK_GOT_ACK || _ack_state == UBX_ACK_GOT_NAK), GPS_CFG_CMD_TIMEOUT_MS);
        } else {
            configureMSG(MSG_CLASS_UBX, MSG_RELPOSNED, 1);
            ptWaitTimeout((_ack_state == UBX_ACK_GOT_ACK || _ack_state == UBX_ACK_GOT_NAK), GPS_CFG_CMD_TIMEOUT_MS);
        }
    }
#endif
