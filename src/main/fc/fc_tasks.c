...
#ifdef USE_DRONECAN
void taskProcessDronecan(timeUs_t currentTimeUs)
{
    dronecanUpdate(currentTimeUs);
#ifdef USE_FLIGHT_COMMANDER_RTK
    flightCommanderRtcmUpdate();
#endif
}
#endif
...
{
#ifdef USE_FLIGHT_COMMANDER_HEADING_FUSION
    flightCommanderExternalCompassUpdate(currentTimeUs);
    flightCommanderHeadingCalibrationUpdate(currentTimeUs);
#endif
...
void fcTasksInit(void)
{
    ...
    // Heading fusion also uses this task for external/DroneCAN mag calibration.
    setTaskEnabled(TASK_COMPASS, true);
#else
    setTaskEnabled(TASK_COMPASS, sensors(SENSOR_MAG));
#endif
#if defined(USE_MAG_MPU9250) || defined(USE_FLIGHT_COMMANDER_HEADING_FUSION)
