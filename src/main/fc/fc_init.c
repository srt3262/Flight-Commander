...
void init(void)
{
...
#ifdef USE_FLIGHT_COMMANDER_COMPASS_ORIENTATION
    flightCommanderCompassOrientationInit();
#endif
#ifdef USE_FLIGHT_COMMANDER_HEADING_FUSION
    flightCommanderHeadingInit();
#endif
#ifdef USE_FLIGHT_COMMANDER_RTK
    flightCommanderRtcmInit();
#endif
    ...
    disableRpmFilters();
    if ((escSensorIsActive() || isDshotTelemetryConfigured()) && (rpmFilterConfig()->gyro_filter_enabled || rpmFilterConfig()->dterm_filter_enabled)) {
        rpmFiltersInit();
